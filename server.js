...
app.get('/api/info-gempa', async (req, res) => {
  ...
  const NEXA_URL =
    'https://api.nexadev.my.id/tools/infogempa/';
   ...
   * gempa terbaru M5.0+.
   *
   * Nexa tetap menjadi sumber utama.
   */
  const BMKG_URL =
  ...
  async function fetchJSON(url, label) {
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `${label} mengembalikan JSON tidak valid`
        );
      }
  ...
  try {
    ...
    console.log('INFO GEMPA: mencoba Nexa API...');
    ...
    if (
      data &&
      data.earthquake &&
      data.earthquake.coordinates
    ) {

      console.log(
        'INFO GEMPA: Nexa berhasil'
      );
    ...
    console.warn(
      'INFO GEMPA: format Nexa tidak sesuai, mencoba BMKG'
    );

  } catch (err) {

    console.error(
      'INFO GEMPA NEXA ERROR:',
      err.message
    );
  ...
  try {
    ...
    console.log(
      'INFO GEMPA: mencoba BMKG fallback...'
    );

    const bmkg = await fetchJSON(
      BMKG_URL,
      'BMKG'
    );

    /*
     * Format autogempa BMKG:
     *
     * Infogempa
     * └── gempa
     */
    ...
    const eq =
      bmkg &&
      bmkg.Infogempa &&
      bmkg.Infogempa.gempa;

    if (!eq) {

      throw new Error(
        'Format BMKG tidak dikenali'
      );
    ...
    /*
     * Konversi koordinat Indonesia:
     * LS = latitude negatif
     * LU = latitude positif
     * BT = longitude positif
     * BB = longitude negatif
     */
    ...
    /*
     * BMKG dapat menyediakan shakemap sebagai:
     * Shakemap / Shakemap URL.
     *
     * Jika tidak tersedia, frontend tidak menampilkan
     * gambar kosong.
     */
    ...
    const normalized = {
      ...
      coordinates: {
        ...
        latitude_string: latRaw,

        longitude_string: lonRaw
    ...
    console.log(
      'INFO GEMPA: BMKG fallback berhasil'
    );
    ...
    return res
      ...
      .json({
        ...
        message:
          'NexaDev dan BMKG tidak dapat diakses',
...
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      defaultSrc: ["'self'"],
      scriptSrc: ["'self'", "'unsafe-inline'", "'unsafe-eval'", "'wasm-unsafe-eval'", "blob:", "https://cdn.jsdelivr.net", "https://cdnjs.cloudflare.com", "https://unpkg.com"],
      styleSrc: ["'self'", "'unsafe-inline'", "https://fonts.googleapis.com", "https://unpkg.com"],
      fontSrc: ["'self'", "https://fonts.gstatic.com", "data:"],
      imgSrc: ["'self'", "data:", "blob:", "https:"],
      connectSrc: ["'self'", "ws:", "wss:", "https://cdn.jsdelivr.net", "https://staticimgly.com", "https://cdnjs.cloudflare.com", "https://unpkg.com", "blob:"],
      workerSrc: ["'self'", "blob:"],
      childSrc: ["'self'", "blob:"]
    }
  },
...
app.get('/api/spotifyplay', async (req, res) => {
  try {
    ...
    if (!q) {
      return res.status(400).json({
        status: false,
        error: 'Query Spotify wajib diisi'
      });
    }
    ...
    try {
      ...
      try {
        ...
        return res.status(502).json({
          status: false,
          error: 'Response Spotify upstream bukan JSON',
    ...
    console.error('Spotify search route error:', e.message);

    if (e.name === 'AbortError') {
      return res.status(504).json({
        status: false,
        error: 'Spotify upstream timeout'
      });
    }

    return res.status(500).json({
      status: false,
      error: 'Gagal menghubungi Spotify API'
    });
  }
});

// ===== JAVACHAT SPOTIFY AUDIO PROXY =====
app.get('/api/spotify/audio', async (req, res) => {
  try {
    const rawUrl = String(req.query.url || '').trim();

    if (!rawUrl) {
      return res.status(400).json({
        error: 'URL audio wajib diisi'
      });
    }
    ...
    try {
      ...
      return res.status(400).json({
        error: 'URL audio tidak valid'
      });
    }

    // Proxy hanya boleh menuju host audio Spotify yang dikenal.
    const allowedHosts = [
      'rapid.spotidown.app'
    ];
    ...
    if (!allowedHosts.includes(target.hostname)) {
      return res.status(403).json({
        error: 'Host audio tidak diizinkan'
      });
    }
    ...
    try {
      ...
      // Teruskan Range dari browser agar HTML5 audio
      // mendapatkan response 206 Partial Content.
      ...
      if (!upstream.ok && upstream.status !== 206) {
        return res.status(upstream.status).json({
          error: 'Gagal mengambil audio dari upstream',
          status: upstream.status
        });
      }

      const upstreamType =
        String(upstream.headers.get('content-type') || '').toLowerCase();

      // Upstream kadang mengirim application/octet-stream.
      // Untuk Spotify playback, gunakan MIME audio yang sesuai.
      ...
      if (e.name === 'AbortError') {
        return res.status(504).json({
          error: 'Audio upstream timeout'
        });
      }

      console.error('Spotify audio proxy error:', e.message);
    ...
    console.error('Spotify audio route error:', e.message);

...
(function(){
  ...
  document.querySelectorAll('[data-close="badgeModal"]').forEach(function(b){ b.onclick = function(){ modal.classList.add('hidden'); }; });
  modal.onclick = function(e){ if (e.target === modal) modal.classList.add('hidden'); };
  ...
  function render() {
    ...
    var activeBadge = badges.find(function(b){ return b.id === active; });
    ...
    badges.forEach(function(b){
      ...
      card.onclick = function(){
        if (!isOwned) {
          ...
          socket.emit('badge-buy', { badgeId: b.id }, function(res){
          ...
          socket.emit('badge-set', { badgeId: null }, function(res){
            if (res && res.error) return alert('Error: ' + res.error);
            if (window.__me) window.__me.customBadge = null;
            render();
          });
        } else {
          socket.emit('badge-set', { badgeId: b.id }, function(res){
            if (res && res.error) return alert('Error: ' + res.error);
  ...
  btn.onclick = function(){ modal.classList.remove('hidden'); render(); };
  socket.on('me', function(u){ window.__me = u; if (!modal.classList.contains('hidden')) render(); });
})();
